...
We will continue assuming that:

- Firms know their own marginal abatement cost
- Regulators observe firm-level emissions
...
Does this error matter for policy design?
...
ggplot() +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 18, color = "#ffffff"),
    axis.text.y = element_text(size = 18, color = "#ffffff"),
...
.center[
![steep vs flat md curves](`r knitr::fig_chunk("prices_vs_quantities_nodwl", "png")`)
]
...
The only difference between the two plots is how steep the MD is relative to the MAC, and subsequently the policy that the regulator sets

Welfare loss for taxes is given by .red[A], welfare loss for permits is given by .blue[B]
...
Steep MD: optimal .hi[quantity] of emissions is inelastic with respect to the true MAC

--

Think about the corner case of a vertical MD
...
Flat MD: optimal .hi[price] of emissions is inelastic with respect to the true MAC

--

Think about the corner case of constant MD
