...
We will continue assuming that firm's know their own marginal abatement cost
...
Then we look at abatement cost uncertainty with know damage functions

We will mainly be focused on efficiency outcomes and want to understand which policy delivers the least welfare loss and why?
...
First suppose the regulator estimates $-C'(E)$ correctly and underestimates marginal damages: $\tilde{D}'(E) < D'(E) \,\, \forall E$
...
Define welfare loss as the difference in total social costs at $\tilde{E}$ and that would have occured at $E^*$:
\begin{align}
WL &= [D(\tilde{E}) + C(\tilde{E})] - [D(E^*) - C(E^*)] \notag\\
  &= [D(\tilde{E}) -D(E^*)] - [ C(\tilde{E})- C(E^*)]
\end{align}

This is equivalent to the area under the marginal curves between the two emission levels:
...
ggplot() +
  ...
  annotate("text", size = 8, label = "D'(E)", x = 4, y = 2.5) +
  annotate("text", size = 8, label = expression(tilde(D)*minute (E)), x = 3, y = 4.5) +
...
![Uncertain MD with permits](`r knitr::fig_chunk("uncertain_damages_tax", "png")`)
]

.pull-right[

With a tax scheme the regulator sets the tax as a function of her target emissions $\tau(\tilde{E})$
...
Suppose the regulator estimates $D'(E)$ correctly and underestimates marginal damages: $-\tilde{E}'(E) < -C'(E) \,\, \forall E$
...
With a tax, the regulator sets a price $\tau(\tilde{E})$ per unit of emissions, and the firms choose the quantity of emissions where $\tau{\tilde{E}} = C'(\tilde{E})$ which causes total emissions to be $E(\tau)$
