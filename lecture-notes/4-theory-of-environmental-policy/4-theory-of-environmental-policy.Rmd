...
<!-- It's just firm $j$'s .hi-blue[permit demand] -->

<!-- -- -->

<!-- Aggregate demand for permits is then: -->
<!-- $$E(\sigma) = \sum_{j=1}^J e_j(\sigma)$$ -->
<!-- --- -->

<!-- # Auctioned pollution permits -->

<!-- and the price $\sigma$ that clears the market equates supply of permits $L$ and demand for permits: -->
<!-- $$L = \sum_{j=1}^J e_j(\sigma)$$ -->

<!-- This equation (supply = demand) defines the market equilibrium -->
...
<!-- Suppose firm $j$ is given $\bar{e}_j$ permits, their total cost is: -->
...
Suppose we subsidize a firm $\xi$ for each unit pollution is below some baseline level $\hat{e}_j$, its total costs are now
...
The firm's FOC condition is then:
$$-C'_j(e_j) = \xi$$

The per-unit abatement subsidy has the same behavioral effect as a per-unit emission tax: firms set MAC equal to the subsidy
...
# Aggregate marginal abatment cost
...
The sum horizontally: $E = 6 - 2\times MAC$
...
So $MAC = 3 - {1 \over 2} E$ is only defined for $P \leq 2, E \geq 4$
