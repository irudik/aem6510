...
For now we assume that these costs are separable (i.e. don't depend on other stuff) so we can focus solely on costs of abating pollution
...
Let $\hat{e}_j$ be the firm's freely chosen abatement level, so that $C_j(\hat{e}_j) = 0$
...
Define the .hi-blue[marginal abatement cost] as:
...
2. Weak abatement cost convexity / weakly increasing MACs is a reasonable approximation of piecewise constant MAC functions, which is what many MACs look like empirically
