...
$$C(x,e) = \min_{l_1,\dots,l_k} \left\{ \sum_{k=1}^K w_k l_k + \lambda \underbrace{\left[ x - f(l_1,\dots,l_K) \right]}_{\text{production tech}} + \mu \underbrace{\left[e-g(l_1,\dots,l_K)\right]}_{\text{emission tech}} \right\}$$
...
## Assumption Set 1 (general model)
--
...
$\hat{e}^x$ is the freely chosen level of emissions at output level $x$
...
2. MAC is increasing in abatement (decreasing in emissions)
...
## Assumption Set 2 (specific model)

We will be directly linking emissions to output in the specific model so we make these assumptions:
...
The MC of abatement is the forgone marginal profits from reducing emissions
...
When facing an emission tax $\tau$ a competitive firm's problem in the .hi[specific case] is:
...
$${dE\over d\tau} = {J \delta^2 \over P'(X)J - C''(x)} < 0$$
...
Assume the regulation is binding in the .hi-red[general case] (i.e. it actually affects firm behavior), then the firm will always set $e = \alpha x$
...
.hi-blue[Total cost and marginal cost are higher under a relative standard]
