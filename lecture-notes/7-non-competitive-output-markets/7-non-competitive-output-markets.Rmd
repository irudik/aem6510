...
Preview: now we have .hi-blue[two] distortions in our market, pollution and market power

---

# Environmental policy under monopoly

Suppose we have an industry with a single firm that generates a local pollutant
...
This illustrates the monopolists decision rule: MR = MC where MR consists of changes in price for .hi-blue[inframarginal units] $(P'(X)X)$
...
Assume the inverse demand function $P(X)$ is decreasing, and for all $X>0$:
$${P''(X) \over P'(X)}X > -1$$
...
$$P'(X^m)dt - C'(X^m) - \delta D'(\delta X^m) \times {d X^m \over d\tau} = 0$$

--

Recognize $P'(X^m)dt - C'(X^m) = \tau\delta - P'(X^m)X^M$ from the firm FOC, now we can solve for $\tau$!

---

# Environmental policy under monopoly

Rearranging gives us the optimal tax rate is characterized by:
...
If we plug it into the firm FOC it is the same as the regulators welfare-maximizing FOC
...
It represents how a change in emissions induces by the tax changes the degree to which the monopolist exercises market power
...
We do this by totally differentiating the firm FOCs wrt $\tau$:
$$P'(X)X + P(X) = C_X(X,E) \qquad -C_E(X,E) = \tau$$
to give us that (dropping function arguments):
...
When you have fewer instruments than degrees of freedom you are in a .hi-blue[second-best world]
