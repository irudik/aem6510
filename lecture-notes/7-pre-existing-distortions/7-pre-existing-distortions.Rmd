...
where the second equality comes from substituting out the govt's budget constraint
...
Next, differentiate the FOCs wrt $\tau$
...
Household $X$ FOC:
$$u_XX {\partial X \over \partial \tau} + u_{XN}{\partial N \over \partial \tau} = {\partial p \over \partial \tau}$$

$N$ FOC:
$$u_XN {\partial X \over \partial \tau} + u_{N}{\partial N \over \partial \tau} = 0$$

Firm $X$ FOC:
$$C''(X) {\partial X \over \partial \tau} = {\partial p \over \partial \tau} - 1$$
...
It's the .hi-blue[marginal revenue effect]: the amount by which emission tax revenue changes when $X$ changes, scaled into welfare units by $\lambda$
...
Here $MRE < 0$ because ${\partial X \over \partial \tau} < 0$ $\rightarrow$ the additional revenue from an increase in $\tau$ lets us reduce labor taxes and reduce the distortion, this reduces the total marginal cost of abatement
