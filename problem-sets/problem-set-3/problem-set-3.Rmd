...
3. In `storms`: create a single variable from the year, month, day, hour variables; then convert it into a `date` variable using `lubridate`
...
6. For each storm: calculate the average wind speed over all observations, and then calculate how much damage was caused per unit of wind speed in a new variable named `damage_per_mph`
